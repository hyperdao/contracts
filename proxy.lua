...
let function can_distribute(self:table,wishedAmount:string,addedAmount:int)
    let sn_wishedAmount = safemath.safenumber(wishedAmount)
    ...
    let cdcContractAddr_object:object = import_contract_from_address(tostring(self.storage.cdcContractAddr)) ---get the cdc object
	let liquidationRatio= cdcContractAddr_object:liquidationRatio('')
	let sn_liquidationRatio = safemath.safenumber(liquidationRatio)
	
	let feederContractAddr:object = import_contract_from_address(tostring(self.storage.feederContractAddr))
	let price = feederContractAddr:getPrice('')
	let sn_price = safemath.safenumber(price)
    
    let sn_totalvalue_wished = safemath.number_multiply(sn_price,sn_wishedAmount)
	let sn_addedAmount = safemath.safenumber(addedAmount)
	let sn_totalvalue_collectal = safemath.number_multiply(sn_price,sn_addedAmount)
	
	let sn_ratio = safemath.number_div(sn_totalvalue_collectal,sn_totalvalue_wished)
	if safemath.number_lt(sn_ratio,sn_liquidationRatio) then
...
let function mint(self:table,address:string,amount: string)
    let stableTokenAddrContract:object = import_contract_from_address(tostring(self.storage.stableTokenAddrContract))
...
function M:init()
	...
	self.storage.feederContractAddr =''
...
function M:init_config(arg:string)
	...
	self.storage.admin = tostring(parsed[1])
	self.storage.symbol = tostring(parsed[2])
	self.storage.feederContractAddr = tostring(parsed[3])
	self.storage.cdcContractAddr = tostring(parsed[4])
	self.storage.stableTokenAddr = tostring(parsed[5])
